...
module.exports = async function (params, { ethers, upgrades, network }) {
  ...
  try {
    ...
    if (isDevelopment(network) || (network.name != 'mainnet' && !isLayer2(network))) {
      // deploy dai mock
    ...
    const governance = await upgrades.deployProxy(Governance, [cap.address, isLayer2(network)], { unsafeAllowCustomTypes: true });
