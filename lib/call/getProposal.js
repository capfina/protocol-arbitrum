...
module.exports = async function (params, hre) {
  ...
  const l2StartBlock = await findL2BlockNumber(proposal.startBlock, hre);

  console.log('startBlock:', proposal.startBlock);
  console.log('l2StartBlock:', l2StartBlock);

  const events = await governance.getPastEvents('ProposalCreated', {
    filter: { id: [ proposalId ] },
    fromBlock: (l2StartBlock - 50000) < 0 ? 'earliest' : l2StartBlock - 50000,
    toBlock: l2StartBlock + 1000
  })
