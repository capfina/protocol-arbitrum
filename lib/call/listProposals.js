...
module.exports = async function (params, hre) {
  ...
  for(let lastIndex = proposalCount ; lastIndex > 0; lastIndex-=5) {
    ...
    // console.log(proposalsBatch);
    const statesBatch = await Promise.all(toFetch.map(i => proposalState({ proposalId: proposalsBatch[proposalsBatch.length - i].id, proposal: proposalsBatch[proposalsBatch.length - i], governance, hre })));
    ...
    const minL2Block = await findL2BlockNumber(minBlock, hre);
    const maxL2Block = await findL2BlockNumber(maxBlock, hre);

    console.log('minBlock:', minBlock, 'maxBlock:', maxBlock);
    console.log('minL2Block:', minL2Block, 'maxL2Block:', maxL2Block);
    const events = await governance.getPastEvents('ProposalCreated', {
      filter: { id: toFetch },
      fromBlock: (minL2Block - 50000) < 0 ? 'earliest' : minL2Block - 50000,
      toBlock: maxL2Block
    })
